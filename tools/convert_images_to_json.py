...
def write_entry(folder, image_path, caption_path, image_filename, intern_imgs_path):
...
def process_filename(args):
    filename, caption_extension, dirpath, intern_imgs_folder = args
    ...
    for image_extension in ['.jpg', '.png', '.jpeg', 'webp', '.JPEG', '.JPG']:
        ...
        if path.exists(image_path):
            try:
                return write_entry(dirpath, image_path, Path(dirpath).joinpath(filename), image_filename + image_extension, intern_imgs_folder)
...
def main():
    ...
    with open(data_info_path, 'w') as json_file:
        for (dirpath, dirnames, filenames) in walk(input_folder):
            ...
            jobs = [(filename, caption_extension, dirpath, intern_imgs_folder) for filename in filenames]
            json_entries = process_map(process_filename, jobs, max_workers=32)
    
        # remove the none entries
        json_entries = [json_entry for json_entry in json_entries if json_entry != None]
